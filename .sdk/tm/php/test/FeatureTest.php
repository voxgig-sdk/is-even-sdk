...
class FtCtrl extends IsEvenControl
{
...
class FtClient
{
    ...
    private IsEvenUtility $utility;

    public function __construct(IsEvenUtility $utility, array $options)
    {
    ...
    public function get_utility(): IsEvenUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IsEvenContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IsEvenContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IsEvenConfig::make_feature($name);
    ...
    private static function buildUrl(IsEvenSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IsEvenError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IsEvenResponse($fetched);
        }
    ...
    private function fail(IsEvenContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IsEvenContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
