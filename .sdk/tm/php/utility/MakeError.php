...
// IsEven SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class IsEvenMakeError
{
    public static function call(?IsEvenContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IsEvenContext([], null);
        }
        $op = $ctx->op ?? new IsEvenOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IsEvenResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IsEvenError) ? $err->msg : (string)$err;
        $msg = "IsEvenSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new IsEvenError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof IsEvenError) {
