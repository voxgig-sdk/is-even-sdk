...
The Is Even API checks whether a given integer is even or odd.

Learn more about Is Even API at [isevenapi.xyz](https://isevenapi.xyz/).
...
const numberparity = await client.NumberParity().load({ number: 1 })
// numberparity is the NumberParity entity, populated with mock data
...
| Go CLI | `github.com/voxgig-sdk/is-even-sdk/go-cli` | `go install github.com/voxgig-sdk/is-even-sdk/go-cli/cmd/is-even@latest` |
| Go MCP server | `github.com/voxgig-sdk/is-even-sdk/go-mcp` | `go get github.com/voxgig-sdk/is-even-sdk/go-mcp@latest` |
...
// Load a specific numberparity (returns a NumberParity)
...
// Load a specific numberparity (returns the ENTITY; call data_get() for the record; throws on error)
$numberparity = $client->NumberParity()->load(["number" => 1]);
...
# Load a specific numberparity (returns the ENTITY; call data_get for the record)
numberparity = client.NumberParity.load({ "number" => 1 })
puts numberparity
```
