...
boru-driven command-line client **and** interactive REPL for the IsEven
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/is-even-cli)
make build
...
./is-even-cli load 1 number_parity            # {id:1} shorthand
./is-even-cli load '{id:1}' number_parity       # explicit match map

# 5. Override the API base URL for a single call
IS_EVEN_BASE=https://api.example.com ./is-even-cli load 1 number_parity

# 6. No arguments -> interactive REPL
./is-even-cli
is-even> load 1 number_parity
is-even> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/is-even-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/is-even-cli load 1 number_parity
   ```
...
```sh
./is-even-cli load 1 number_parity          # scalar shorthand for {id:1}
./is-even-cli load '{id:1}' number_parity     # explicit match map
```
...
./is-even-cli load 1 number_parity
```
...
Run with no arguments to open a REPL (prompt `is-even>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `number_parity`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/is-even-cli`. |
...
(`./is-even-cli <expr>`) and interactively (the REPL), and expressions compose the
