...
MCP server exposing the IsEven SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `is-even_list` | `entity`, optional `query` map | First page of records as JSON |
| `is-even_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `is-even_list` and
`is-even_load` tools become available in new sessions.
