...
[MCP](https://modelcontextprotocol.io) server exposing the IsEven SDK as
two agent tools — `is-even_list` and `is-even_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/is-even-mcp)
make build

# 2. Provide credentials via the environment
export IS_EVEN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user is-even \
  -- /absolute/path/to/is-even-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./is-even-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// is-even_list: first page of records
{ "entity": "number_parity" }
{ "entity": "number_parity", "query": { } }

// is-even_load: one record by id
{ "entity": "number_parity", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/is-even-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user is-even \
     -- "$PWD"/dist/*/is-even-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `is-even_list` and `is-even_load` tools now appear
   in new sessions. Ask the agent to *"list number_parity using is-even"*
   and it calls `is-even_list` with `{"entity":"number_parity"}`.
...
```sh
export IS_EVEN_APIKEY=sk_live_xxx            # API key
export IS_EVEN_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `is-even_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `is-even_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IS_EVEN_APIKEY` | API key sent with every request. |
| `IS_EVEN_BASE` | Optional override of the API base URL. |
...
```sh
./is-even-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"is-even_load","arguments":{"entity":"number_parity","query":{"id":1}}}}'
```
