...
// IsEven SDK NumberParity entity
...
class NumberParityEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new NumberParityEntity($this->_client, $opts);
    }

    /**
     * @param NumberParity|array $args NumberParity data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IsEvenHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return NumberParity|array The current NumberParity data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of NumberParity fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IsEvenHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of NumberParity fields).
     */
    ...
    /**
     * Load a single NumberParity.
     *
     * @param NumberParityLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed NumberParityLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return NumberParity|array The loaded NumberParity as an assoc-array at the
     *   SDK boundary; throws IsEvenError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IsEvenHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
