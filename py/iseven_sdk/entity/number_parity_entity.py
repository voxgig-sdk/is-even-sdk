...
from iseven_sdk.iseven_types import (
    NumberParity,
...
class NumberParityEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return NumberParityEntity(self._client, opts)
    ...
    def data_get(self) -> NumberParity:
    ...
    def match_get(self) -> NumberParity:
    ...
    def load(self, reqmatch=None, ctrl=None) -> NumberParity:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.NumberParity().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
