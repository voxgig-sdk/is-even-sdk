...
class NumberParityEntity
  ...
  def make
    opts = @_entopts.dup
    NumberParityEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IsEvenHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [NumberParity, Hash] the current NumberParity data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IsEvenHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of NumberParity fields)
  def match_get
  ...
  #
  # @param reqmatch [NumberParityLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.NumberParity.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [NumberParity, Hash] the loaded NumberParity; raises IsEvenError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IsEvenHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
