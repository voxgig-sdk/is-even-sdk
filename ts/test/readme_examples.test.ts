...
//     (\`new IsEvenSDK(...)\` / \`IsEvenSDK.test(...)\` become
//     \`IsEvenSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"number_parity":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IsEvenSDK(...)\` and any
// \`IsEvenSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IsEvenSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IsEvenSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
