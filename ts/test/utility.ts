...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.IS_EVEN_TEST_LIVE ||
    'TRUE' === process.env.IS_EVEN_TEST_OVERRIDE
  ) {
  ...
  m.IS_EVEN_TEST_EXPLAIN = process.env.IS_EVEN_TEST_EXPLAIN || m.IS_EVEN_TEST_EXPLAIN
